...
function Home() {
    ...
    // let filterD=data.matchDetails.filter(singleMatch=>singleMatch["matchDetailsMap"])

    // console.log('filterD', filterD);


    async function FetchMData(params) {
        ...
        try {
            ...
            let filterD = result.matchDetails.filter(singleMatch => singleMatch["matchDetailsMap"])
            setMatchData(filterD)
        } catch (error) {
            console.error(error);
        }
    }
    ...
    return (
        <div>
            {matchData.map((data, i) => {
                return data.matchDetailsMap.match.map((singleMatch) => (
                    <Link to={`/matchDetail/${singleMatch.matchInfo.matchId}`}>
                        <h1>{singleMatch.matchInfo.team1.teamSName} vs {singleMatch.matchInfo.team2.teamSName}
                            , {data.matchDetailsMap.key},{singleMatch.matchInfo.status}</h1>
                    </Link>
                ))
                //  return <h1 key={i}>{data.matchDetailsMap.key}</h1>
            })}
        </div>
    )
}
